...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 3 && res != 1) {  // проверка на корректность количества переменных в грепе
...
int parser(int argc, char**argv, flg_t *flg, char *pattern) {
    ...
    while ((c = getopt(argc, argv, "e:ivclnhsof:")) != -1) {
        switch (c) {
            ...
            case 'o' : { flg->o = 1; break; }
            case 'f' : { flg->f = 1; sprintf(pattern, "%s", optarg); break; }
            default : {
                res = 1;
...
void output(flg_t *flg, FILE *fp, regex_t reg, char *file, int file_count) {
    ...
    while (fgets(text, 99999 - 1, fp) != NULL) {
        ...
        if (file_count && status == 0 && !flg->l && !flg->c && !flg->h) {  // если файлов больше чем 1
            printf("%s:", file);  // выводим название кажого файла
        }

        if ((status == 0 && !flg->v) || (status == REG_NOMATCH && flg->v)) match = 1;
...
int s_p(char *pattern, char *temp) {
    ...
    if (fp != NULL) {
        ...
        while ((c = fgetc(fp)) != EOF) {
            if (c == '\n' || c == 13) pattern[i++] = '|';  // 13 - символ возврата картеки
            if (c != '\n' && c != 13) pattern[i++] = c;
        }
